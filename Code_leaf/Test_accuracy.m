...
hold on;
figure(3),plot(y,x,'-bo');

saveas(figure(3),'NBGraph.jpg');

hold off;
legend('KNN Accuracy','Ground Truth');
%%
load('Train_NB.mat');
[predict_label] = predict(mdl_nb,feature_matrix_test_accu);
cMat2= confusionmat(predict_label,Tar);

[Acc] = Result(predict_label,Tar);
%
fprintf('NB Accuracy:=%f Percentage\n',Acc*100);
% fprintf('Sensitivity:=%f\n',sensitivity);   % Print the Results on the Command Window
% fprintf('Specificity:=%f\n',specificity);
for i=1:10
res(i)=cMat2(i,i);
...
figure(2),plot(y,res,'-mo');
grid on;
x=[length(find(Tar==1)),length(find(Tar==2)),length(find(Tar==3)),length(find(Tar==4)),length(find(Tar==5)),length(find(Tar==6)),length(find(Tar==7)),length(find(Tar==8)),length(find(Tar==9)),length(find(Tar==10))];
hold on;
figure(2),plot(y,x,'-ko');

%saveas(plot(y,res,'-mo'),plot(y,x,'-ko')),'KNNGraph.jpg');

hold off;
legend('NB Accuracy','Ground Truth');